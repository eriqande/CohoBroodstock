...
You get the package like this:
```r
install.packages("related", repos="http://R-Forge.R-project.org")
```
But it requires some gfortran compilation.  On Mac OSX you can try downloading gfortran-6.11 from
[https://cran.r-project.org/bin/macosx/tools/](https://cran.r-project.org/bin/macosx/tools/).  It seems
to compile with that...but fails with a seg fault.

So, from the same page, for R 3.6 I also downloaded clang7 and installed it.
